...
let call_btns = document.querySelector('.call_btns');
let userContactList = document.querySelectorAll('.user_contact_list');
...
let profile_contact_name = document.querySelector('.profile_contact h5');
...
userContactList.forEach((e, index) => {
    e.setAttribute('id', `contact_btns${index}`);
    e.addEventListener('click', () =>{
        if(e.id === `contact_btns${index}`){
            contact_chat_area.style.display = 'block';
        }
        console.log(e.firstElementChild.childNodes[1].src);
        console.log(e.children[1]);
    })
});

profile_contact_img.addEventListener('click', () => {
    contact_chat_area.style.display = "none";
